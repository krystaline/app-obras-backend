...
def fill_parte_obra_pymupdf(
    template_pdf_path: str, output_pdf_path: str, data: ParteImprimirPDF
):
    try:
        ...
        if data.idParteERP is not None:
            # Ensure it's a string
            page.insert_text(
                fitz.Point(66.5, 45.8),
                str(data.idParteERP),
        ...
        if data.lineas:
            for i, activity in enumerate(data.lineas):
                ...
                if activity.descripcion is not None:
                    page.insert_text(
                        fitz.Point(col_x_actividades, current_y),
                ...
                if activity.cantidad is not None:
                    page.insert_text(
                        fitz.Point(col_x_cant, current_y),
                        str(activity.cantidad),
                        fontsize=font_size_table,
                        fontname=font_name,
                        color=text_color,
                    )
                ...
                if activity.unidadMedida is not None:
                    page.insert_text(
                        fitz.Point(col_x_unid, current_y),
                        activity.unidadMedida,
                        fontsize=font_size_table,
                        fontname=font_name,
                        color=text_color,
                    )
        ...
        send_email_with_attachment(
            output_pdf_path, str(data.proyecto) + "_parte_" + str(data.idParteERP)
        )
