...
class ParteRecibidoPost(BaseModel):
    ...
    idParteAPP: int  # SIEMPRE viene lleno
    firma: str = ""
    lineas: List[
        LineaPedidoPost
    ] = []  # Asumiendo que LineaPedidoPost es para recibir datos de la UI
...
class ParteImprimirPDF(BaseModel):
    idParteERP: int = Field(
        ..., alias="idParteERP"
    )  # Alias si el campo de DB es diferente, ej. n_parte
    idParteAPP: int = Field(
        ..., alias="idParteAPP"
    )  # Alias si el campo de DB es diferente, ej. n_parte
    proyecto: str
    oferta: int = Field(..., alias="oferta")  # Si en DB se llama diferente, ajusta
    jefe_equipo: str
    telefono: str
    fecha: datetime.date  # Considera cambiar a date si la DB lo guarda como fecha
    contacto_obra: str
    comentarios: Optional[str] = (
        None  # 'None' en lugar de un string vacío si es opcional y puede ser NULL
    )
