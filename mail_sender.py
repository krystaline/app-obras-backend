...
def get_access_token():
    ...
    if "access_token" in result:
        ...
        raise Exception(
            f"Fallo al adquirir el token: {result.get('error')}, {result.get('error_description')}"
        )
...
def send_email_with_attachment(file_path, filename):
    ...
    if recipient_emails_str:
        try:
            ...
            for email in parsed_emails:
                to_recipients_list.append({"emailAddress": {"address": email}})
        except json.JSONDecodeError:
            print(
                f"Error decoding RECIPIENT_EMAILS: {recipient_emails_str}. Ensure it's a valid JSON array string."
            )
    ...
    try:
        response = requests.post(
            send_mail_url, headers=headers, data=json.dumps(email_data)
        )
